...
def create_segment(obj, iteration_number):
    ...
    cur = S.cursor()
    cur.execute('insert into segment (severity) values (?)', (obj['severity'],))
...
def insert_into_existing_segment(delegate, obj, iteration_number):
    ...
    for _x in QUANTITATIVE_FEATURES:
...
def get_delegates():
    ...
    """
    _d = S.cursor()
    _d.execute('select _id_segment, severity from segment')
    all_id_segments = _d.fetchall()
    structured_delegates = []
    for _id_segment in all_id_segments:
        _r = []
        ...
        for row in _r2:
            _deg.setdefault(row['feature'], []).append(dict(value=row['value'],
                                                            frequency=row['frequency']))
        # seg is the final delegate that contains all the needed information
        seg = {'_id_segment': _id_segment[0], 'delegate': _deg, 'severity': _id_segment[1]}
        structured_delegates.append(seg)
    return structured_delegates
...
def segment_one(obj, iteration_number):
    ...
    structured_delegates = get_delegates_by_solution(obj['severity'])
...
def segment_all(obj_array, iteration_number):
    ...
    segment for _it and store _it)
    """
    # TODO divide obj_array according to obj's severities
    for severity in POSSIBLE_SOLUTIONS:
        # get delegates of all the concerned segments
        new_delegates = get_delegates_by_solution(severity)
        for obj in obj_array:
            obj['segmented'] = False
            # if there is no segment
            if obj['severity'] == severity and not new_delegates:
                print('_id_case', obj['_id_case'])
                obj['segmented'] = True
                new_delegate = create_segment(obj, iteration_number)
                new_delegates.append(new_delegate)
            elif obj['severity'] == severity and new_delegates:
                print('_id_case', obj['_id_case'])
                i = 0
